...
1. 管理画面から： 管理 > キャッシュをクリア
1. コマンドラインから： EspoCRM インストールディレクトリで次のいずれかのコマンドを実行：
2. php rebuild.php または php command.php clear-cache
...
- 管理画面 → 設定 → 言語
- 各ユーザーのプロフィール設定 → 言語 ここで Japanese (Japan) または 日本語 を選択して保存してください。
